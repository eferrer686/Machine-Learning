...
class NeuronalNet(object):
    ...
    def createNeuralNet(self):
        ...
        X_input = tf.placeholder(shape=[None, self.n_inputs_dim], dtype=tf.float32)
        # Agrega el placeholder para y, salidas
        y = tf.placeholder(shape=[None, self.n_output], dtype=tf.float32)
        ...
        #Reporta qué es una función de activación tf.nn.elu
            #Es una funcion de activiacion que a diferencia de la sigmoide esta es lineal en valores positivos y curvo en negativo
        hidden1 = fully_connected(self.X_input,
                                self.n_hidden,
        ...
        logits = fully_connected(hidden1,
                                self.n_output,
    ...
    def train(self):
        ...
        _, cost = self.sess.run([self.train_op,self.cost], feed_dict={  self.X_input: self.X,
                                                                        self.y:self.Y
                                                                    })        
        return cost
...
net = NeuronalNet(sess, X_train, Y_train, n_hidden=1)
